...
def test( i_loss_func,
  ...
  with torch.no_grad():
    for (l_x, l_y) in io_data_loader:
      ...
      l_n_correct = torch.tensor((l_prediction.argmax(1) == l_y).type(torch.float).sum().item())
      torch.distributed.all_reduce(l_n_correct, op = torch.distributed.ReduceOp.SUM)
      l_n_correct = l_n_correct / float(l_size)
      

      #l_n_total += l_y.numel()
      #print(l_n_total)
      #print(len(l_y[0]))
      l_n_total = torch.tensor(l_y.numel())
      torch.distributed.all_reduce(l_n_total, op = torch.distributed.ReduceOp.SUM)
      l_n_total = l_n_total / float(l_size)
      #l_n_total = l_n_total / l_size
